...
let windowRef; let quitting = false; let alwaysOnTop = true;
...
ipcMain.on('pet:menu', () => Menu.buildFromTemplate([{ label: '打开今天待办', click: () => openDate(today()) }, { label: '待办事项历史记录', submenu: historyMenu() }, { type: 'separator' }, { label: alwaysOnTop ? '取消始终置顶' : '始终置顶', type: 'checkbox', checked: alwaysOnTop, click: item => setPetAlwaysOnTop(item.checked) }, { type: 'separator' }, { label: '退出自嘲熊', click: () => app.quit() }]).popup({ window: windowRef }));
